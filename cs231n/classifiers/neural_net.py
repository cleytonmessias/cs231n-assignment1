...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    grad_h_relu = np.dot(dscores, W2.T)
    grad_h_relu[h < 0] = 0  # the derivate of ReLU
    dw1 = np.dot(X.T, grad_h_relu)
    db1 = np.sum(grad_h_relu, axis=0)
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in range(num_iters):
      ...
      #########################################################################
      test_index = np.random.choice(num_train, batch_size, replace=True)
